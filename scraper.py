...
RETAILERS = [
    ...
    },
    {
        ...
        "max_pages": 15,
        "use_playwright": True,
        "ignore_sitemap": True,  # Sitemap atlanip direkt kategorilere gidilecek
        "concurrent_workers": 1
    },
    {
        "name": "Kozmela",
...
def extract_ingredients(soup):
    ...
    for el in soup.find_all(["div", "p", "span", "li", "section"]):
        ...
        if "İçindekiler" not in txt and "Ingredients" not in txt and "INCI" not in txt.upper():
            continue
        candidate = clean_text(txt.replace("İçindekiler:", "").replace("Ingredients:", "").replace("İçindekiler", ""))
        if _looks_like_ingredient_list(candidate): candidates.append(candidate)
...
def parse_product_page(soup, p_res):
    ...
    if not price:
        for sel in [".price-sales", ".price-undiscounted", ".discount-price", ".current-price", "[class*='discounted']", "[itemprop='price']", "[class*='price']"]:
            for el in soup.select(sel):
                if not _is_valid_price_context(el): continue
    ...
    if not image_url:
        ...
        if img_el:
            ...
            if candidate and "logo" not in candidate.lower(): image_url = candidate
...
def process_store(store):
    ...
    if use_playwright: _dbg(f"[{store['name']}] Playwright modu AKTIF")
    ...
    if not store.get("ignore_sitemap", False):
        product_urls = set(try_sitemap_urls(scraper, base_domain, stats))

    if not product_urls:
        print(f"[{store['name']}] Sitemap atlandi veya bulunamadi, kategori sayfalari taranacak")
        for cat_url in store["start_urls"]:
            urls = extract_product_urls_from_category(
                scraper, cat_url, stats, store.get("pagination_param", "page"), store.get("max_pages", 15), use_playwright=use_playwright
            )
    ...
    # Playwright kullanılan siteler için iş parçacığı (Thread) KULLANMIYORUZ. Sıralı tarıyoruz.
    if use_playwright:
        for idx, url in enumerate(product_urls, 1):
            if time.time() - start_time > MAX_STORE_RUNTIME_SECONDS:
                ...
                break
            try:
                _, p_res, error = fetch_product_page(url, referer=base_domain, use_playwright=True)
                ...
                if existing.data:
                    ...
                    ins = supabase.table("products").insert({
                        "brand_id": brand_id, "name": name, "slug": unique_slug, "category": "Kozmetik",
                        "image_url": image_url, "original_inci_text": inci_text
                    }).execute()
    ...
    # Playwright kullanılmayan siteler için eski çoklu iş parçacığı (Thread) yöntemi
    else:
        workers = store.get("concurrent_workers", CONCURRENT_WORKERS)
        with ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_url = {executor.submit(fetch_product_page, url, referer=base_domain, use_playwright=False): url for url in product_urls}
